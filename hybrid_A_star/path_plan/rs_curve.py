...
def calc_optimal_path(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=STEP_SIZE):
    ...
    # 找到最短的路径
    Ls = [path.L for path in paths]
    mini = Ls.index(min(Ls))
    path_choice = paths[mini]

    # import matplotlib.pyplot as plt
    # plt.figure()
    # for i, path in enumerate(paths):
    #     plt.plot(path.x, path.y, '-.', label=f'path_{i}')
    # plt.plot(path_choice.x, path_choice.y, 'k-', label=f'path_choice')
    # plt.legend()
    # plt.axis("equal")
...
def set_path(paths, lengths, ctypes):
    ...
    if path.L >= MAX_LENGTH and path.L < 0.01:
        return paths

    # assert path.L >= 0.01, f"path length is too small, {lengths}"
    paths.append(path)
