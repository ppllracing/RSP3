...
class hybrid_a_star:
    def __init__(self,
        ...
        _, self.h_value_list = self.heuristic.compute_path(node_x=park_map.case.x0, node_y=park_map.case.y0)  # 先用DIjkstra计算起点到所有节点的最短路径
        
        # 从heuristic中获取初始状态下的启发
        self.occ_heuristic = np.zeros_like(self.park_map.cost_map)
        for h_value in self.h_value_list:
            x_id, y_id = self.park_map.convert_xy_to_id(h_value.grid_x, h_value.grid_y)
            self.occ_heuristic[x_id, y_id] = h_value.distance

        #二阶段考虑世界坐标系的满足运动学约束的节点数
        # default settings
        self.global_index = 0
        self.config = config
    ...
    def expand_node(self, current_node: Node) -> queue.PriorityQueue:
        ...
        next_index = int(2 * self.config['steering_angle_num'])  # 分别探索前进和后退的节点， 所以要乘2
        for i in range(next_index):
            ...
            # 计算按照当前前轮转角和速度所能达到的位置
            travle_distance = speed * self.dt
            theta_ = current_node.theta + self.vehicle.max_v * np.tan(steering_angle) / self.vehicle.lw * self.dt
            ...
            if find_opennode == False:
                ...
                # 将轨迹离散进行碰撞检测
                # 若其中任意一点发生碰撞，则将该节点放入closed_list
                # 若全部路径都没有发生碰撞，则将该节点放入open_list
                for i in range(math.ceil(self.dt / self.ddt)):
                    ...
                    # collision check
                    collision = self.collision_checker.check(node_x=x_i, node_y=y_i, theta=theta_i)
    ...
    def try_rs_curve(self, current_node: Node):
        ...
        # collision check
        for path_x, path_y, path_theta in zip(rs_path.x, rs_path.y, rs_path.yaw):
            collision = self.collision_checker.check(node_x=path_x, node_y=path_y, theta=rs_curve.pi_2_pi(path_theta))
