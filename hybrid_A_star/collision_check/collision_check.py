...
class two_circle_checker(collision_checker):
    ...
    def check(self, node_x, node_y, theta) -> bool:
        ...
        Rd_max = 0.5 * np.sqrt(((v.lr+v.lw+v.lf)/2)**2 + (v.lb**2))
        Rd_min = (v.lr+v.lw+v.lf)/4
        Rd = (Rd_min + Rd_max) / 2
        ...
        # plt.plot(node_x + (v.lw + v.lf) * np.cos(theta), node_y + (v.lw + v.lf) * np.sin(theta), 'ko')
        # plt.plot(node_x + (v.lw + v.lf) * np.cos(theta) + v.lb/2 * np.cos(theta+np.pi/2), node_y + (v.lw + v.lf) * np.sin(theta) + v.lb/2 * np.sin(theta+np.pi/2), 'ko')
        # plt.plot(node_x + (v.lw + v.lf) * np.cos(theta) - v.lb/2 * np.cos(theta+np.pi/2), node_y + (v.lw + v.lf) * np.sin(theta) - v.lb/2 * np.sin(theta+np.pi/2), 'ko')
        # plt.plot(node_x - v.lr * np.cos(theta), node_y - v.lr * np.sin(theta), 'ko')
        # plt.plot(node_x - v.lr * np.cos(theta) + v.lb/2 * np.cos(theta+np.pi/2), node_y - v.lr * np.sin(theta) + v.lb/2 * np.sin(theta+np.pi/2), 'ko')
        # plt.plot(node_x - v.lr * np.cos(theta) - v.lb/2 * np.cos(theta+np.pi/2), node_y - v.lr * np.sin(theta) - v.lb/2 * np.sin(theta+np.pi/2), 'ko')
        # theta_ = np.linspace(0, 2*np.pi, 100)
        # x = Rd * np.cos(theta_)
        # y = Rd * np.sin(theta_)
        # plt.plot(front_circle[0] + x, front_circle[1] + y, 'b')
        # plt.plot(front_circle[0], front_circle[1], 'bo')
        # plt.plot(rear_circle[0] + x, rear_circle[1] + y, 'g')
...
class distance_checker(collision_checker):
    ...
    def check(self, node_x, node_y, theta) -> bool:
        ...
        def compute_k_b(point_1, point_2):
            # k = (y_2 - y_1) / (x_2 - x_1)
            k = (point_2[1] - point_1[1]) / (point_2[0] - point_1[0] + 1e-6)
            # b = y_1 - k * x_1
