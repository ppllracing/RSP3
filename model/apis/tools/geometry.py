...
def calculate_birds_eye_view_parameters(x_bounds, y_bounds, z_bounds, dtype=torch.float64):
    """
    Parameters
    ----------
    ...
    """
    bev_resolution = torch.tensor([row[2] for row in [x_bounds, y_bounds, z_bounds]], dtype=dtype)  # 分别提取分辨率
    bev_start_position = torch.tensor([row[0] + row[2] / 2.0 for row in [x_bounds, y_bounds, z_bounds]], dtype=dtype)  # 提取开始点
    bev_dimension = torch.tensor([(row[1] - row[0]) / row[2] for row in [x_bounds, y_bounds, z_bounds]], dtype=torch.int)  # 计算各方向点数
